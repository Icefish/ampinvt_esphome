...
class ampinvtsensor : public PollingComponent,
                      public Sensor,
                      public UARTDevice {
public:
  ampinvtsensor(UARTComponent *parent)
  ...
  Sensor *ampinvt_op_status =
      new Sensor(); // 1 bit ~ byte 3 (0=Normal, 1=Abnormal - Battery Automatic
                    // Recognition Error)
  Sensor *ampinvt_battery_status =
      new Sensor(); // 1 bit ~ byte 3 (0=Normal, 1=Over Discharge Protection)
  Sensor *ampinvt_fan_status =
      new Sensor(); // 1 bit ~ byte 3 (0=Normal, 1=Fan Failure)
  Sensor *ampinvt_overheat_status =
      new Sensor(); // 1 bit ~ byte 3 (0=Normal, 1=Over Temperature Protection)
  Sensor *ampinvt_dcoutput_status =
      new Sensor(); // 1 bit ~ byte 3 (0=Normal, 1=DC Output SHort / Over
                    // Current Protection)
  Sensor *ampinvt_inttemp1_status =
      new Sensor(); // 1 bit ~ byte 3 (0=Close, 1=Fault)
  Sensor *ampinvt_inttemp2_status =
      new Sensor(); // 1 bit ~ byte 3 (0=Close, 1=Fault)
  Sensor *ampinvt_exttemp_status =
      new Sensor(); // 1 bit ~ byte 3 (0=Close, 1=Fault)
  Sensor *ampinvt_chg_status =
      new Sensor(); // 1 bit ~ byte 4 (0=Not Charging, 1=Charging)
  ...
  Sensor *ampinvt_chgcurrentlimit_status =
      new Sensor();                                  // 1 bit ~ byte 4 (1=True)
  Sensor *ampinvt_chgderating_status = new Sensor(); // 1 bit ~ byte 4 (1=True)
  Sensor *ampinvt_remoteprohibchg_status =
      new Sensor();                                 // 1 bit ~ byte 4 (1=True)
  Sensor *ampinvt_pvovervolt_status = new Sensor(); // 1 bit ~ byte 4 (1=True)
  Sensor *ampinvt_chgoutputrelay_status =
      new Sensor(); // 1 bit ~ byte 5 (0=Close, 1=Open)
  Sensor *ampinvt_loadoutput_status =
      new Sensor(); // 1 bit ~ byte 5 (0=Close, 1=Open)
  Sensor *ampinvt_fanrelay_status =
      new Sensor(); // 1 bit ~ byte 5 (0=Close, 1=Open)
  Sensor *ampinvt_overchgprotect_status =
      new Sensor(); // 1 bit ~ byte 5 (0=Normal, 1=Overcharge Protection)
  Sensor *ampinvt_overvoltprotect_status =
      new Sensor(); // 1 bit ~ byte 5 (0=Normal, 1=Overvoltage Protection)
  // Byte sensors follow
    ...
    while (available() > 0) {
      ...
      if (bytes.size() == 37) {
        ...
        uint32_t generation_total_value =
            int((unsigned char)(bytes[24]) << 24 |
                (unsigned char)(bytes[25]) << 16 |
                (unsigned char)(bytes[26]) << 8 | (unsigned char)(bytes[27]));
